...
BASELINE = date(2000, 1, 1)  # sentinel: pairs present at first run (not "new")
...
def record_snapshot(df):
    """First run establishes a silent baseline; only pairs first seen on LATER
    runs get today's date and count as genuinely new."""
    if "first_seen" not in st.session_state:
        st.session_state.first_seen = {}
        st.session_state.initialised = False
    seen = st.session_state.first_seen
    first_run = not st.session_state.initialised
    stamp = BASELINE if first_run else date.today()
...
def new_in_window(df, seen, start_d, end_d):
    result = {}
    for _, r in df.iterrows():
        ld = _as_date(r["listed_date"])  # OKX real date
        if ld is None:
            ld = _as_date(seen.get((r["exchange"], r["pair"], r["category"])))
        if ld is not None and ld != BASELINE and start_d <= ld <= end_d:
...
st.caption(
    ...
    "(baseline is set on first run, and resets if the app restarts). 'Convert' has no "
    "public listed pairs. Perpetuals are available on OKX & KuCoin only."
)
